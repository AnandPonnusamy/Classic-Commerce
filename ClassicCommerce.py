...
root=Tk()
root.title("ClassicCommerce.in")
root.geometry("1360x500")
import tkinter as tk
from tkinter import messagebox

Heading=LabelFrame(root,bd=2,relief="groove",bg="bisque2")
...
appliances_list=[]
name=Label(Heading,text="Classic Commerce",font="arial 20 bold",bg="brown4",fg="gray100").grid(row=0,column=1)
tagline=Label(Heading,text="Find it, Love it, Buy it!",font="magneto 16 italic",fg="gold",bg="black").grid(row=0,column=2,padx=280)
...
def save_invoice(text):
    # Confirm whether to save the invoice
    op = messagebox.askyesno("Invoice Saving Confirmation", "Do you want to save the invoice in a file?")
    
    if op:
        # Generate a unique filename based on the current time
        t = datetime.now()
        s = f"{t.day:02d}{t.month:02d}{t.year}{t.hour:02d}{t.minute:02d}{t.second:02d}"
        filename = f"Invoices/{s}.txt"
        ...
        try:
            ...
            messagebox.showinfo("Invoice Saving Status", f"Invoice is saved successfully as a text document with name {s}.txt")
        except IOError as e:
...
def SportsGymCall():
    ...
    def AddS4():
        global sportsgym_list
        op=messagebox.askyesno("Pur+chase Confirmation","Are you sure that you want to add this item to the cart?")
        if op:
...
def Bill():
    ...
    if op:
        ...
        total_price=electronics_price+sportsgym_price+appliances_price
        discount=[0,0,0]
...
bill_gen_button=Button(Heading,bd=4,text="Proceed to Checkout",font="times 15 bold",bg="green",fg="white",activebackground="purple",command=Bill)
